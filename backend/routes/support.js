...
router.get('/my', authenticate, async (req, res) => {
  const messages = await db.prepare('SELECT * FROM support_messages WHERE user_id = ? ORDER BY created_at DESC').all(req.user.id);
...
router.post('/', authenticate, async (req, res) => {
  ...
  const id = uuid();
  const ticketNumber = (await db.prepare('SELECT COALESCE(MAX(ticket_number),0) as n FROM support_messages').get()).n + 1;
  await db.prepare('INSERT INTO support_messages (id, user_id, type, subject, message, ticket_number) VALUES (?,?,?,?,?,?)')
    .run(id, req.user.id, kind, subject.trim(), message.trim(), ticketNumber);
  await logActivity(req, 'support.create', 'support', id, { customerName: `${user.first_name} ${user.last_name}`, type: kind, subject: subject.trim(), ticketNumber });

  res.status(201).json(await db.prepare('SELECT * FROM support_messages WHERE id = ?').get(id));
});
