...
router.put('/bookings/:id/status', (req, res) => {
  ...
  if (status === 'completed') {
    ...
    const alreadyPending = db.prepare("SELECT id FROM change_requests WHERE entity_type='booking' AND entity_id=? AND status='pending'").get(req.params.id);
    if (alreadyPending) return res.status(400).json({ error: 'A completion request for this job is already pending verification.' });

    const requestId = createChangeRequest('booking', 'update', req.params.id, { status, completionNotes: completionNotes || '' }, req.user.id);
    ...
    const jobDetails = db.prepare(`
      ...
      WHERE b.id = ?
    `).get(req.params.id);
    const installer = db.prepare('SELECT first_name, last_name FROM users WHERE id = ?').get(req.user.id);
    ...
    const coordinators = db.prepare("SELECT id FROM users WHERE role='employee' AND position='booking_coordinator'").all();
    coordinators.forEach(c => notifyUser(c.id, 'booking.complete_requested', 'Completion Needs Verification', message, '/admin/approvals'));
