...
router.get('/threads', (req, res) => {
  ...
  const threads = partnerIds.map(({ partner_id }) => {
    const partner = db.prepare('SELECT id, first_name, last_name, position, staff_code, role FROM users WHERE id = ?').get(partner_id);
    ...
    const unread = db.prepare('SELECT COUNT(*) as c FROM staff_messages WHERE sender_id = ? AND recipient_id = ? AND is_read = 0').get(partner_id, req.user.id).c;
...
router.get('/thread/:userId', (req, res) => {
  const partner = db.prepare('SELECT id, first_name, last_name, position, staff_code, role FROM users WHERE id = ?').get(req.params.userId);
  ...
  db.prepare('UPDATE staff_messages SET is_read = 1 WHERE sender_id = ? AND recipient_id = ? AND is_read = 0').run(req.params.userId, req.user.id);
...
router.post('/', (req, res) => {
  ...
  const recipient = db.prepare('SELECT id, first_name, last_name FROM users WHERE id = ?').get(recipientId);
  if (!recipient) return res.status(404).json({ error: 'Recipient not found' });

  const id = uuid();
  db.prepare('INSERT INTO staff_messages (id, sender_id, recipient_id, body) VALUES (?,?,?,?)').run(id, req.user.id, recipientId, body.trim());

  const sender = db.prepare('SELECT first_name, last_name FROM users WHERE id = ?').get(req.user.id);
  notifyUser(recipientId, 'message.new', `New message from ${sender.first_name} ${sender.last_name}`, body.trim().slice(0, 100), `/admin/messages?with=${req.user.id}`);
